  ...
  async () => {
    ...
    try {
      await Instance.provide({
        directory: tmp.path,
        fn: async () => {
          await AppRuntime.runPromise(Skill.Service.use((svc) => svc.all()))
          await AppRuntime.runPromise(Command.Service.use((svc) => svc.list()))
        },
      })
      ...
      await Instance.provide({
        ...
        fn: async () => {
          ...
          const command = await AppRuntime.runPromise(Command.Service.use((svc) => svc.get("second-skill")))
          const build = await AppRuntime.runPromise(Agent.Service.use((svc) => svc.get("build")))
          ...
          expect(skills.map((skill: Skill.Info) => skill.name).sort()).toEqual(["first-skill", "second-skill"])
