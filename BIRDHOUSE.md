...
We maintain a set of commits on top of the latest tagged upstream release. The goal is a clean history where every commit passes CI independently. The current base is `v1.4.1`.
...
```bash
GIT_SEQUENCE_EDITOR=true git rebase -i --autosquash v1.4.1
```

Replace `v1.4.1` with the current upstream base tag if it has changed.
